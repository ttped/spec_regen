...
def find_content_start_page(
    ...
    Returns a dict with:
        ...
        - page_classifications: list of {page, type} for debugging
    """
    ...
    if total_pages == 1:
        ...
        if stub_redirect:
            ...
            return {
                ...
                "page_classifications": [{'page': page_id, 'type': 'STUB'}]
            }
        
        # Not a stub - classify the single page
        page_type = get_page_type_with_llm(page_text, llm_config)
        print(f"    Page {page_id}: {page_type}")
        ...
        return {
            ...
            "is_stub": False,
            "stub_redirect": None,
            "page_classifications": [{'page': page_id, 'type': page_type}]
        }
    
    # Multi-page document: classify first N pages (INCLUDING page 1)
    pages_to_classify = pages[:max_pages_to_check]
    
    print(f"  - Classifying pages 1-{len(pages_to_classify)} with {max_workers} workers...")
