...
def extract_caption_text(
    ...
    YOLO bboxes are in docs_images/ pixel space; OCR bboxes are in render_raw
    pixel space.  These may differ in resolution, so we normalize both to 0-1
    range before comparing — the same approach that works in section_processor's
    overlap filter.
    ...
    Args:
        ...
        yolo_image_width: Width of the image YOLO ran on (for normalization)
        yolo_image_height: Height of the image YOLO ran on (for normalization)
    ...
    # --- Determine OCR coordinate space dimensions ---
    # Try image_meta.render_raw first (what Tesseract actually ran on)
    ocr_width = None
    ocr_height = None
    
    image_meta = page_data.get('image_meta', {})
    if image_meta:
        render_raw = image_meta.get('render_raw', {})
        if render_raw:
            ocr_width = render_raw.get('width_px')
            ocr_height = render_raw.get('height_px')
        if not ocr_width or not ocr_height:
            canonical = image_meta.get('canonical', {})
            if canonical:
                ocr_width = canonical.get('width_px')
                ocr_height = canonical.get('height_px')
    
    # Fallback: infer from the max extent of OCR word coordinates
    if not ocr_width or not ocr_height:
        max_right = max(
            (lefts[i] + widths[i] for i in range(min(len(lefts), len(widths)))),
            default=0
        )
        max_bottom = max(
            (tops[i] + heights[i] for i in range(min(len(tops), len(heights)))),
            default=0
        )
        if max_right > 0 and max_bottom > 0:
            ocr_width = max_right
            ocr_height = max_bottom
    
    if not ocr_width or not ocr_height:
        return ""
    
    # --- Normalize YOLO caption bbox to 0-1 ---
    cx1, cy1, cx2, cy2 = caption_bbox
    norm_cx1 = cx1 / yolo_image_width
    norm_cy1 = cy1 / yolo_image_height
    norm_cx2 = cx2 / yolo_image_width
    norm_cy2 = cy2 / yolo_image_height
    
    # Collect words whose normalized bboxes overlap sufficiently with the caption region
    caption_words = []
    for i in range(min(len(texts), len(lefts), len(tops), len(widths), len(heights))):
        ...
        # Skip degenerate entries (non-word Tesseract levels often have 0 dimensions)
        if ww <= 0 or wh <= 0:
            continue
        
        # Normalize OCR word bbox to 0-1
        norm_wl = wl / ocr_width
        norm_wt = wt / ocr_height
        norm_wr = (wl + ww) / ocr_width
        norm_wb = (wt + wh) / ocr_height
        
        # Calculate intersection in normalized space
        inter_l = max(norm_wl, norm_cx1)
        inter_t = max(norm_wt, norm_cy1)
        inter_r = min(norm_wr, norm_cx2)
        inter_b = min(norm_wb, norm_cy2)
        
        if inter_r <= inter_l or inter_b <= inter_t:
            continue
        
        inter_area = (inter_r - inter_l) * (inter_b - inter_t)
        word_area = max((norm_wr - norm_wl) * (norm_wb - norm_wt), 1e-10)
        
        if inter_area / word_area >= overlap_threshold:
            word_text = str(texts[i]).strip()
            if word_text and word_text != '-1':
                caption_words.append((norm_wl, norm_wt, word_text))
...
def is_likely_hole_punch(
    ...
    """
    Detect hole-punch false positives: small, roughly square marks near
    the left or right page margin.

    Args:
        bbox_pixels: (x1, y1, x2, y2) in pixel coordinates
        image_width: Full page image width
        image_height: Full page image height
        max_area_ratio: Maximum bbox area as fraction of page area (default 0.5%)
        max_aspect_ratio: Maximum ratio of long side to short side
        margin_fraction: How close to the edge counts as "margin" (default 8%)

    Returns:
        True if the detection is likely a hole punch and should be discarded
    """
    ...
    # Must be roughly square (not elongated text)
    if aspect_ratio > max_aspect_ratio:
        return False

    # Must be near the left or right edge
    near_left = x1 / image_width < margin_fraction
    near_right = x2 / image_width > (1.0 - margin_fraction)
...
def run_detection_on_image(
    ...
    filtered_assets = []
    for d in asset_detections:
        if is_likely_hole_punch(d.bbox_pixels, img_width, img_height):
            print(f"      [Filtered] Likely hole punch on page {page_number}: "
                  f"bbox={d.bbox_pixels}, class={d.class_name}, conf={d.confidence:.2f}")
        else:
            filtered_assets.append(d)
    asset_detections = filtered_assets
    ...
    for detection in asset_detections:
        ...
        if has_caption and caption_bbox and raw_ocr_dir:
            caption_text = extract_caption_text(
                ...
                yolo_image_width=img_width,
                yolo_image_height=img_height
            )
